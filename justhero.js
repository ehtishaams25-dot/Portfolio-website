...
for(let i=0;i<44;i++){const m=new THREE.Mesh(ringG,ringM);m.position.set((Math.random()-0.5)*120,(Math.random()-0.5)*80,-(300+Math.random()*TUN));m.userData.rx=0;m.userData.ry=0;scene.add(m);allObj.push({mesh:m,kind:'ring',got:false});}
...
let score=0,lvl=0,speedMult=1.0,curSpeed=0;
...
function tick(){
    ...
    for(let i=0;i<allObj.length;i++){const o=allObj[i],m=o.mesh;
        if(o.kind==='bad'){
            // Tumble on all 3 axes — each model spins differently
            m.rotation.x += m.userData.rx || 0;
            m.rotation.y += m.userData.ry || 0.008;
            m.rotation.z += m.userData.rz || 0;
            m.position.y = (m.userData.baseY||0) + Math.sin(performance.now()*(m.userData.bobS||0.001) + (m.userData.bobO||0)) * 4;
            // Collision (all models, larger radius, deeper z check)
            if(cam.position.z < m.position.z + 10 && cam.position.z > m.position.z-30){
                ...
                if(Math.sqrt(dx*dx+dy*dy)<25 && !o.hit){
                    o.hit=true; snd('damage');
                    score = 0; lvl = 0; speedMult = 1.0; curSpeed = Math.min(curSpeed, 1.0);
            ...
            // Recycle obstacle ahead when it passes behind camera
        ...
        if(o.kind==='ring'){m.rotation.x+=m.userData.rx;m.rotation.y+=m.userData.ry;}
        if(o.kind==='ring'&&!o.got){if(cam.position.z<m.position.z&&cam.position.z>m.position.z-18){const dx=cam.position.x-m.position.x,dy=cam.position.y-m.position.y;if(Math.sqrt(dx*dx+dy*dy)<20){o.got=true;score++;scoreEl.textContent=score;snd('collect');checkLevel();gsap.fromTo(scoreBoxEl,{scale:1.2,color:'#fff'},{scale:1,color:'var(--beige)',duration:0.4});const fm=m.material.clone();fm.emissiveIntensity=4;m.material=fm;gsap.to(m.scale,{x:0,y:0,z:0,duration:0.4});setTimeout(()=>{m.visible=false;},400);}}}
        if(m.position.z>cam.position.z+100&&o.kind==='ring'){o.got=false;m.visible=true;m.scale.set(1,1,1);m.material=ringM;m.position.set((Math.random()-0.5)*120,(Math.random()-0.5)*80,cam.position.z-TUN-Math.random()*500);}
    }
